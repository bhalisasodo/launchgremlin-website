...
export default function PhysicalCardPreview({ card, qrDataUrl }) {
  ...
  return (
    <div className="w-full flex flex-col items-center space-y-6">
      ...
      <div className="w-full flex flex-wrap items-center justify-between gap-3 bg-zinc-900/80 border border-zinc-800 p-3 rounded-2xl">
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
          <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider mr-2 hidden sm:inline">Finish:</span>
          {[
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {showBleedGuides ? 'Hide Bleed Lines' : 'Print Guides'}
          </button>
      ...
      <div
        className="w-full max-w-[440px] aspect-[1.75/1] cursor-pointer perspective-[1200px]"
        onClick={() => setIsFlipped(!isFlipped)}
      ...
      >
        ...
        >
          {/* FRONT OF CARD */}
          <div
            className={`absolute inset-0 backface-hidden rounded-2xl border-2 p-6 flex flex-col justify-between overflow-hidden transition-all ${currentStyles.front}`}
          >
            ...
            <div className="flex items-start justify-between relative z-10">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-xl overflow-hidden bg-zinc-950/80 border border-zinc-700/60 flex items-center justify-center font-black text-base shadow-inner">
                  {card.avatarUrl ? (
                ...
                <div>
                  <h3 className="font-black text-sm tracking-tight leading-snug">{card.fullName || card.companyName}</h3>
                  <p className={`text-[11px] font-semibold tracking-wide ${currentStyles.accentText}`}>{card.jobTitle || 'Executive'}</p>
                </div>
              </div>

              {card.verified && (
                <div className={`px-2 py-0.5 rounded-full border text-[9px] font-bold uppercase tracking-wider flex items-center gap-1 ${currentStyles.foilBadge}`}>
            ...
            {card.tagline && (
              <div className="relative z-10 my-auto py-1">
                <p className="text-[10.5px] opacity-80 leading-relaxed line-clamp-2 italic">
                  "{card.tagline}"
                </p>
              </div>
            )}
            ...
            <div className="relative z-10 flex items-end justify-between border-t border-white/10 pt-2.5 text-[10px] opacity-85">
              <div className="space-y-0.5">
                {card.companyName && card.fullName && (
                  <span className="font-bold block uppercase tracking-wider text-[9.5px] opacity-90">{card.companyName}</span>
                )}
                {card.email && <div className="font-mono text-[9px] opacity-75">{card.email}</div>}
              </div>
              <div className="text-right space-y-0.5 font-mono text-[9px]">
                {card.phone && <div>{card.phone}</div>}
                {card.websiteUrl && <div className="text-emerald-400">{card.websiteUrl.replace(/^https?:\/\//, '')}</div>}
              </div>
            </div>
          </div>
          ...
          {/* BACK OF CARD */}
          <div
            className={`absolute inset-0 backface-hidden rotate-y-180 rounded-2xl border-2 p-6 flex flex-col justify-between overflow-hidden transition-all ${currentStyles.front}`}
          >
            ...
            <div className="flex items-center justify-between relative z-10">
              <div>
                <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-emerald-400">CONNECT INSTANTLY</span>
                <h4 className="font-extrabold text-sm text-white">Scan or Tap NFC</h4>
              </div>
            ...
            <div className="flex items-center justify-between gap-4 relative z-10 my-auto">
              <div className="space-y-1.5 max-w-[200px]">
                <p className="text-[10.5px] text-zinc-300 leading-relaxed font-medium">
                  Point camera or tap phone to view portfolio, schedule meetings, and download contact card.
                </p>
                <div className="text-[9.5px] font-mono text-emerald-400 truncate">
                  launchgremlin.com/c/{card.slug || 'card'}
                </div>
              </div>

              {/* QR Code Container */}
              <div className="p-2 bg-white rounded-xl shadow-lg shrink-0 border border-zinc-300 flex items-center justify-center">
                {qrDataUrl ? (
                  <img src={qrDataUrl} alt="QR Code" className="w-20 h-20 object-contain" />
                ) : (
                  <div className="w-20 h-20 flex items-center justify-center bg-zinc-100 text-zinc-900">
            ...
            {/* Back Footer */}
            <div className="relative z-10 flex items-center justify-between border-t border-white/10 pt-2 text-[9px] text-zinc-400 font-mono">
              <span>Standard ISO / US 3.5" × 2"</span>
              <span>100% Recyclable • NFC Integrated</span>
            </div>
          </div>
        </div>
      </div>
